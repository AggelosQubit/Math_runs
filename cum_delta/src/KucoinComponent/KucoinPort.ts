...
import { transformHistoricalTrade, getCurrentCumDelta, transformNextOrderTradedSides, getNumberOfDeltaConsumed, nullifyCDandNbOfDeltaConsumed } from './KucoinAdapter';
import { SpotMarketTradedOrder, KucoinSubscriptionObject, HistoricalTrades } from '../CustomTypesComponent/CustomTypes'
import { app } from '../cum_delta';
...
app.get('/kucoin/CumDelta/reInit', (req, res) => {
    nullifyCDandNbOfDeltaConsumed();
    res.sendStatus(200);
});
app.get('/kucoin/CumDelta/update',
    (req, res) => {
        if (req.body["pair"] !== undefined && req.body["pair"] !== "") {
            if (soc === undefined || soc.readyState === Websocket.CLOSED) {
                ...
                soc.on('open', () => {
                    ...
                    let objToSend = {
                        "pair": req.body["pair"],
                        'CurrentCumDelta': getCurrentCumDelta(),
...
app.get('/kucoin/CumDelta/history', async (req, res) => {
